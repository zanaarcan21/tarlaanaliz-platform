...
# KR-015: Capacity policy and estimation helpers for planning orchestration.
...
def estimate_days_for_area(
    ...
    *,
    ...
    cap = estimate_daily_capacity(requested=daily_capacity_donum, policy=policy)
    if cap <= 0:
        return float("inf")
    return float(donum) / float(cap)
...
def estimate_capacity(
    ...
    *,
    ...
    cap = estimate_daily_capacity(requested=daily_capacity_donum, policy=policy)
    days = estimate_days_for_area(donum, daily_capacity_donum=cap, policy=policy)
    return CapacityEstimate(donum=float(donum), effort_units=effort, daily_capacity_donum=cap, estimated_days=days)
